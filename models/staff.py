...
class Staff:
    ...
    def add_page_header(self):
        ...
        txt_wdgt = tk.Label(self.frame, text="Sales Reps", bg=self.my_bg, fg=self.colour2, font=("lucida", 20, "bold")).grid(row=1, column=2, columnspan=8)
        ...
        add_new_btn = tk.Button(self.frame, text="Add new", bg=self.colour2, font=('lucida', 10), fg=self.my_bg, activebackground=self.my_bg, activeforeground=self.colour2, cursor="hand2", command=self.add_new_form).grid(row=2, column=6, pady=25, sticky='e')


    def add_header(self):
        header = ["Name", "Email", "Phone number", "Position", "Department"]
    ...
    def add_new_form(self):
         ...
         add_form.title('New Rep form')
         r_name = tk.StringVar()
         r_email = tk.StringVar()
         r_phone = tk.StringVar()
         r_pos = tk.StringVar()
         r_dpt = tk.StringVar()
         my_vars = [r_name, r_email, r_phone, r_pos, r_dpt]
         ...
         name_entry = tk.Entry(add_form, relief="ridge", textvariable=r_name, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=0, column=1, pady=25, columnspan=2, sticky='w', padx=10)
         ...
         email_entry = tk.Entry(add_form, relief="ridge", textvariable=r_email, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=1, column=1, pady=25, columnspan=2, sticky='w', padx=10)
         ...
         phone_entry = tk.Entry(add_form, relief="ridge", textvariable=r_phone, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=2, column=1, pady=25, columnspan=2, sticky='w', padx=10)
         pos_wdgt = tk.Label(add_form, text="Position", bg=self.my_bg, fg=self.colour2, font=("lucida", 10, "bold"), justify="left").grid(row=3, column=0, pady=25, sticky='w', padx=10)
         pos_entry = tk.Entry(add_form, relief="ridge", textvariable=r_pos, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=3, column=1, pady=25, columnspan=2, sticky='w', padx=10)
         dpt_wdgt = tk.Label(add_form, text="Department", bg=self.my_bg, fg=self.colour2, font=("lucida", 10, "bold"), justify="left").grid(row=4, column=0, pady=25, sticky='w', padx=10)
         dpt_entry = tk.Entry(add_form, relief="ridge", textvariable=r_dpt, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=4, column=1, pady=25, columnspan=2, sticky='w', padx=10)
         done_btn = tk.Button(add_form, text="Done", bg=self.colour2, font=('lucida', 10), fg=self.my_bg, activebackground=self.my_bg, activeforeground=self.colour2, cursor="hand2", command=lambda:self.add_new(add_form, my_vars)).grid(row=5, column=0, columnspan= 3, pady=25)
         add_form.mainloop()
    ...
    def add_new(self, top, my_vars):
        ...
        if ("" in my_vars):
            ...
            messagebox.showinfo('Sales Rep Creation', 'Successful!')
            top.destroy()
            for w in self.frame.winfo_children():
                if (len(w.grid_info()) > 0):
                    if(w.grid_info()['row'] >=4):
    ...
    def update_form(self, event):
        ...
        for w in self.frame.winfo_children():
            if (len(w.grid_info())>0):
                if (w.grid_info()['row'] == row and w.grid_info()['column'] >= 2 and w.grid_info()['column'] < 7):
                        r_details.append(w.cget("text"))
        up_form=Toplevel(self.frame, background=self.my_bg, width=400, height=400)
        up_form.title('Sales Reps Update form')
        up_name = tk.StringVar()
        up_email = tk.StringVar()
        up_phone = tk.StringVar()
        up_pos = tk.StringVar()
        up_dpt = tk.StringVar()
        my_vars = [up_name, up_email, up_phone, up_pos, up_dpt]
        for i in range(0, len(r_details)):
            my_vars[i].set(r_details[i])
        ...
        pos_wdgt = tk.Label(up_form, text="Position", bg=self.my_bg, fg=self.colour2, font=("lucida", 10, "bold"), justify="left").grid(row=3, column=0, pady=25, sticky='w', padx=10)
        pos_entry = tk.Entry(up_form, relief="ridge", textvariable=up_pos, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=3, column=1, pady=25, columnspan=2, sticky='w', padx=10)
        dpt_wdgt = tk.Label(up_form, text="Department", bg=self.my_bg, fg=self.colour2, font=("lucida", 10, "bold"), justify="left").grid(row=4, column=0, pady=25, sticky='w', padx=10)
        dpt_entry = tk.Entry(up_form, relief="ridge", textvariable=up_dpt, bg=self.colour1, fg=self.colour2, borderwidth=2, cursor="tcross", width=56).grid(row=4, column=1, pady=25, columnspan=2, sticky='w', padx=10)
        update_btn = tk.Button(up_form, text="Update", bg=self.colour2, font=('lucida', 10), fg=self.my_bg, activebackground=self.my_bg, activeforeground=self.colour2, cursor="hand2", command=lambda: self.update(up_form, my_vars, r_details[5])).grid(row=5, column=0, columnspan= 3, pady=25)
        up_form.mainloop()
    ...
    def update(self, top, my_vars, id):
        ...
        if ("" in vars):
            ...
            messagebox.showinfo('Sales Rep Update', 'Successful!')
            top.destroy()
            for w in self.frame.winfo_children():
                if (len(w.grid_info()) > 0):
                    if(w.grid_info()['row'] >=4):
